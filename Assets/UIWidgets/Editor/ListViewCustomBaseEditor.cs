...
namespace UIWidgets
{
	...
	public class ListViewCustomBaseEditor : Editor
	{
		...
		protected bool IsListViewCustomWidth = false;
		...
		protected virtual void OnEnable()
		{
			...
			if (!IsListViewCustomWidth)
			{
				IsListViewCustomWidth = DetectGenericType(serializedObject.targetObject, "UIWidgets.ListViewCustomWidth`2");
			}
			if (!IsTileView)
			{
			...
			if (IsListViewCustomWidth)
			{
				var isCanCalculateWidth = false;
				var ourType = serializedObject.targetObject.GetType(); 
				var mi = ourType.GetMethod("IsItemCanCalculateWidth", BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
				if (mi!=null)
				{
					var isCanCalculateWidthFunc = (Func<bool>)Delegate.CreateDelegate(typeof(Func<bool>), serializedObject.targetObject, mi);
					isCanCalculateWidth = isCanCalculateWidthFunc.Invoke();
				}

				if (isCanCalculateWidth)
				{
					if (!Properties.Contains("ForceAutoWidthCalculation"))
					{
						Properties.Add("ForceAutoWidthCalculation");
					}
				}
				else
				{
					if (Properties.Contains("ForceAutoWidthCalculation"))
					{
						Properties.Remove("ForceAutoWidthCalculation");
					}
				}
				if (!Properties.Contains("itemWidth"))
				{
					Properties.Add("itemWidth");
				}
			}
