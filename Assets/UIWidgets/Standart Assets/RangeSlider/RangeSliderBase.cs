...
namespace UIWidgets
{
		...
		where T : struct
	{
		...
		protected void UpdateHandle(RectTransform handle, T value)
		{
			...
			if (IsHorizontal())
			{
				anchorMin.x = (ValueToPosition(value) + handle.rect.width * (handle.pivot.x - 0.5f)) / size[0];
				anchorMax.x = anchorMin.x;
				pos.x = 0;
			}
			else
			{
				anchorMin.y = (ValueToPosition(value) + handle.rect.height * (handle.pivot.y - 0.5f)) / size[1];
