...
namespace UIWidgets
{
	...
	public class ObjectSliding : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
	{
		...
		{
			var cur_pos = IsHorizontal() ? RectTransform.anchoredPosition.x : RectTransform.anchoredPosition.y;

			return Positions.OrderBy(pos => Mathf.Abs(pos - cur_pos)).First();
		}
		...
		protected virtual IEnumerator RunAnimation(bool isHorizontal, float startPosition, float endPosition, bool unscaledTime)
		{
			...
			do
			{
				delta = ((unscaledTime ? Time.unscaledTime : Time.time) - start_time);
