...
namespace UIWidgets
{
	...
	public class Sidebar : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
	{
			...
			if (IsOpen)
			{
				switch (Direction)
				{
					...
					case SidebarAxis.BottomToTop:
						...
						break;
					default:
						throw new NotImplementedException("Unknown sidebar axis: " + Direction);
				}
			}
		...
		protected void AddHandleEvents(GameObject handleObject)
		{
			...
			if (handle==null)
			{
				handle = handleObject.AddComponent<SidebarHandle>();
			}
		...
		protected virtual void ContentScaleDrag(Vector2 delta)
		{
			...
			var rectDelta = startPosition - endPosition;
		...
		{
			var content_size = Content.rect;
			var rect_size = SidebarRect.rect;

			var scale = IsHorizontal()
				? (content_size.width - (rect_size.width * 2 - pos * 2)) / content_size.width
				: (content_size.height - (rect_size.height * 2 - pos * 2)) / content_size.height;

			scale = Mathf.Max(scale, ScaleDownLimit);
		...
		{
			var close = RectOpenPosition(SidebarRect, SidebarPosition, 1.5f);
			var open = RectOpenPosition(SidebarRect, SidebarPosition);
			RectDrag(SidebarRect, open, close, delta * -0.5f);
			ContentDrag(delta);
		}
