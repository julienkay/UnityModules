...
namespace UIWidgets
{
	...
	/// </summary>
	[AddComponentMenu("UI/UIWidgets/ColorPicker Image")]
	public class ColorPickerImagePalette : MonoBehaviour
	{
		...
		protected virtual void SetImage(Image value)
		{
			...
			if (image!=null)
			{
				...
				dragListener = image.GetComponent<OnDragListener>();
				if (dragListener==null)
				{
					dragListener = image.gameObject.AddComponent<OnDragListener>();
				}
		...
		protected int[] Color2TextureCoords(Color color, Texture2D texture, float diff=0f)
		{
			...
			{
				var new_delta = ColorDelta(color, pixels[i]);
				if (new_delta < delta)
				{
					delta = new_delta;
					pos = i;
		...
		protected virtual void UpdateViewReal()
		{
			...
			if ((imageCursor!=null) && (image!=null) && (imageRect!=null) && (GetPixelColorUnderCursor()!=currentColor))
			{
				...
				if ((coords[0]==-1) && (coords[1]==-1))//if coords not found hide cursor
				{
