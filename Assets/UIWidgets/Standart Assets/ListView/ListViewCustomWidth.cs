...
namespace UIWidgets
{
		...
		where TItem: IItemWidth
	{
		...
		public override void ScrollTo(int index)
		{
			...
			var bottom = GetScrollValue() + scrollWidth;
		...
		float GetItemsWidth(int start, int count)
		{
			...
			var width = DataSource.GetRange(start, count).SumFloat(GetItemWidth);
		...
		{
			return Mathf.Max(0, DataSource.GetRange(0, index).SumFloat(GetItemWidth) + (LayoutBridge.GetSpacing() * (index - 1)));
		}
		...
		/// <summary>
		/// Total width of items before specified index.
		/// </summary>
		/// <returns>Width.</returns>
		/// <param name="index">Index.</param>
		float ItemStartAt(int index)
		{
			var width = DataSource.GetRange(0, index).SumFloat(GetItemWidth);

			return width + (LayoutBridge.GetSpacing() * index);
		}

		/// <summary>
		/// Total width of items before and with specified index.
		/// </summary>
		/// <returns>The <see cref="System.Single"/>.</returns>
		/// <param name="index">Index.</param>
		float ItemEndAt(int index)
		{
			var width = DataSource.GetRange(0, index + 1).SumFloat(GetItemWidth);
		...
		{
			var last_visible_index = GetIndexByWidth(GetScrollValue() + scrollWidth);
