...
namespace UIWidgetsSamples
{
	...
	public class TestTreeView : MonoBehaviour
	{
		...
		public void SelectAndScrollTo(TreeNode<TreeViewItem> node)
		{
			...
			// scroll to node with specified speed (points per second)
			//StartCoroutine(ScrollToAnimatedSpeed(index, 3000f));

			// scroll to node with fixed time (seconds)
			StartCoroutine(ScrollToAnimatedTime(index, 0.3f));
		}
		...
		/// </summary>
		/// <param name="node">Node.</param>
		protected virtual void CollapseParentNodes(TreeNode<TreeViewItem> node)
		{
		...
		/// </summary>
		/// <param name="node">Node.</param>
		protected virtual void ExpandParentNodes(TreeNode<TreeViewItem> node)
		{
			foreach (var n in node.Path)
			{
				n.IsExpanded = true;
		...
		/// <summary>
		/// Scroll to node animated with specified speed.
		/// </summary>
		/// <param name="index">Index.</param>
		/// <param name="speed">Speed.</param>
		/// <returns>Animation coroutine.</returns>
		protected virtual IEnumerator ScrollToAnimatedSpeed(int index, float speed)
		{
			var target_pos = Tree.GetItemPosition(index);
			var delta = Mathf.Abs(target_pos - Tree.ScrollRect.content.anchoredPosition.y);

			return ScrollToAnimatedTime(index, delta / speed);
		}

		IEnumerator ScrollToAnimatedTime(int index, float time)
		{
			var target_pos = Tree.GetItemPosition(index);
			var current_position = Tree.ScrollRect.content.anchoredPosition;
			var end_time = Time.time + time;

			while (Time.time <= end_time)
			{
				var pos_y = Mathf.Lerp(current_position.y, target_pos, 1 - (end_time - Time.time) / time);

				Tree.ScrollRect.content.anchoredPosition = new Vector2(current_position.x, pos_y);

				yield return null;
			}

			Tree.ScrollRect.content.anchoredPosition = new Vector2(current_position.x, target_pos);
		}
